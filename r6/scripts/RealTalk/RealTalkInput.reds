...
//  used purely defensively (see RealTalkActionGuard below).
//
...
//
//  Keys (Mod Settings > Real Talk > Controls; defaults):
...
public class RealTalkInputSystem extends ScriptableSystem {
    ...
    //  (RealTalkDialog.reds). Restore = push the captured hubs back AND
    ...
    private func TryOpen(ui: ref<RealTalkUI>, settings: ref<RealTalkSettings>) -> Void {
        ...
        // The full ladder lives in RealTalkGender.reds: persistent state,
    ...
    //  Action guard
    // ------------------------------------------------------------------
    private let actionGuard: ref<RealTalkActionGuard>;

    public func RegisterGuard(player: ref<PlayerPuppet>) -> Void {
        this.actionGuard = new RealTalkActionGuard();
...
public class RealTalkActionGuard {
