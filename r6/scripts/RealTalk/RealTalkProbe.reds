...
// ============================================================================
public class RealTalkFS extends ScriptableService {
    ...
    public func Storage() -> ref<FileSystemStorage> {
        if !this.tried {
            this.tried = true;
            this.storage = FileSystem.GetStorage("RealTalk");
        }
        return this.storage;
    }

    public static func Get() -> ref<RealTalkFS> {
        return GameInstance.GetScriptableServiceContainer()
            .GetService(n"RealTalk.RealTalkFS") as RealTalkFS;
    }
}

public class RealTalkLog extends ScriptableSystem {
    ...
    private func AddLine(text: String) -> Void {
        ...
        let file = storage.GetFile("realtalk.log");
        if !IsDefined(file) {
            return;
        }
        let all: String = "";
    ...
    public static func Get() -> ref<RealTalkLog> {
        return GameInstance.GetScriptableSystemsContainer(GetGameInstance())
            .Get(n"RealTalk.RealTalkLog") as RealTalkLog;
    }
}
...
public static func StLog(const text: String) {
    ...
    FTLog(s"[RealTalk]: \(text)");
    let sink = RealTalkLog.Get();
    if IsDefined(sink) {
        sink.Add(text);
    }
}
