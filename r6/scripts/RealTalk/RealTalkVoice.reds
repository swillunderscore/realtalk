...
//       local TTS server (server/realtalk-tts.py, port 8082).
//    2. The server synthesises the line in the NPC's cloned voice and writes
//       it over r6/audioware/RealTalk/slots/slot_<N>.wav, replying only
...
public class StVoice {
    ...
    public static func SpeakFlat(game: GameInstance, slot: Int32) -> Void {
        ...
        ext.Play(StringToName(s"realtalk_slot_\(slot)"));
    ...
    public static func SilenceFlat(game: GameInstance, slot: Int32) -> Void {
        let ext = GameInstance.GetAudioSystemExt(game);
        if IsDefined(ext) {
            ext.Stop(StringToName(s"realtalk_slot_\(slot)"));
        }
    }
    ...
    public static func Speak(game: GameInstance, npc: wref<NPCPuppet>, slot: Int32) -> Void {
        ...
        if !ext.IsRegisteredEmitter(id, n"RealTalk") {
            ext.RegisterEmitter(id, n"RealTalk");
        }
        ext.PlayOnEmitter(StringToName(s"realtalk_slot_\(slot)"), id, n"RealTalk");
    ...
    public static func Prepare(game: GameInstance, npc: wref<NPCPuppet>) -> Void {
        ...
        let ext = GameInstance.GetAudioSystemExt(game);
        if IsDefined(ext) && !ext.IsRegisteredEmitter(npc.GetEntityID(), n"RealTalk") {
            ext.RegisterEmitter(npc.GetEntityID(), n"RealTalk");
        }
    }
    ...
    public static func Silence(game: GameInstance, npc: wref<NPCPuppet>, slot: Int32) -> Void {
        ...
        let ext = GameInstance.GetAudioSystemExt(game);
        if IsDefined(ext) {
            ext.StopOnEmitter(StringToName(s"realtalk_slot_\(slot)"), npc.GetEntityID(), n"RealTalk", null);
        }
    }
