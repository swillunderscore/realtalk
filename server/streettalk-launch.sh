#!/bin/bash
# ============================================================================
#  StreetTalk launcher wrapper - makes voice (and anything else) seamless.
# ============================================================================
#
...
#
#      /path/to/streettalk-launch.sh %command% -no-tls
#
...
if [ -f "$PIDFILE" ]; then
    ...
    : > "$PIDFILE"
    [ "$STRAYS" = "1" ] && { echo "[streettalk] cleaned up sidecars from a previous session"; sleep 1; }
fi
...
if [ -f "$CONF" ]; then
    while IFS= read -r line; do
        ...
        if pgrep -f "$base" >/dev/null 2>&1; then
            echo "[streettalk] $base already running - not touching it"
            continue
        fi
        echo "[streettalk] starting $base"
        mkdir -p "$DIR/logs"
...
: > "$PIDFILE"
[ "${#PIDS[@]}" -gt 0 ] && echo "[streettalk] stopped ${#PIDS[@]} sidecar(s)"
exit $rc
