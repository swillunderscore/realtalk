...
# (REALTALK_LOG_CONTENT=1) - the structural lines alone diagnosed every bug
# in this file's history, and the stage-direction words are only needed when
# actively extending the animation thesaurus.
# Set by REALTALK_LOG_CONTENT=1, and ALSO switched on per request by the
...
LOG_CONTENT = os.environ.get("REALTALK_LOG_CONTENT", "") == "1"
...
def harvest_examples(slug: str, wems_dir_hint: str = "", alias: str = ""):
    ...
    out_dir = os.path.join(ARGS.game_dir, "r6", "storages", "RealTalk")
...
def cached_gender(voice: str) -> str:
    ...
    if not (voice and ARGS.game_dir):
        return ""
    path = os.path.join(ARGS.game_dir, "r6", "storages", "RealTalk",
...
def find_anim(direction: str, gender: str, held: bool = False) -> str:
    ...
    # explicitly enabled (REALTALK_LOG_CONTENT=1).
    if LOG_CONTENT:
...
def _wiki_fetch(page: str):
    ...
    req = urllib.request.Request(url, headers={
        "User-Agent": "RealTalk-Cyberpunk2077-mod (local character lookup)"})
        ...
        try:
            ...
            if "log" in req:
                ...
                LOG_CONTENT = bool(req.get("log")) or (
                    os.environ.get("REALTALK_LOG_CONTENT", "") == "1")
...
def main():
    ...
    ap.add_argument("--slots", required=True,
                    help="game's r6/audioware/RealTalk/slots directory")
